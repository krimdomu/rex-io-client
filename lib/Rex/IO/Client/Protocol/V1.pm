...
sub update_incident_status {
  ...
  $self->_post( "/incident/$incident_id/status", { status_id => $status_id } )
    ->res->json;
}
...
sub list_incident_messages {
  ...
  my $res =
    decode_json( $self->_list("/incident/$incident_id/message")->res->body );
...
sub add_task_to_host {
  my ( $self, %option ) = @_;
  $self->_post(
    "/service/host/" . $option{host} . "/task/" . $option{task},
    { task_order => ( $option{task_order} || 0 ) }
  )->res->json;
}
...
sub run_task_on_host {
  my ( $self, %option ) = @_;
  $self->_run( "/service/host/" . $option{host} . "/task/" . $option{task} )
    ->res->json;
}
...
sub trigger_reboot {
  my ( $self, $ip ) = @_;
  $self->_post( "/messagebroker/$ip",
    { type => "exec", exec => "/sbin/reboot" } )->res->json;
}
...
sub set_next_boot {
  ...
  $self->_post( "/hardware/$option{server}",
    { os_template_id => $option{boot} } )->res->json;
}
...
sub list_monitoring_item_of_template {
  ...
  my $data =
    decode_json( $self->_list("/monitor/template/$template_id")->res->body );
...
sub get_monitoring_template {
  my ( $self, $template_id ) = @_;
  my $data =
    decode_json( $self->_get("/monitor/template/$template_id")->res->body );
...
sub get_monitoring_item {
  my ( $self, $template_id, $item_id ) = @_;
  my $data = decode_json(
    $self->_get("/monitor/template/$template_id/item/$item_id")->res->body );
...
sub update_monitoring_item {
  ...
  $self->_post( "/monitor/template/$template_id/item/$item_id", \%option )
    ->res->json;
}
